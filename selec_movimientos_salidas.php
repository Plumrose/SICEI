...
<div id="Layer1">
  <div align="center" class="Estilo28">
    <p class="Estilo41">Listado de Movimientos de Salida </p>
    </div>
</div>
<form id="form1" name="form1" method="post" action="selec_movimientos_salidas.php">
  <p>&nbsp;</p>
  <p>&nbsp;</p>
<div id="busqueda">
  <table width="820">
      ...
      <tr>
        ...
        <?php
           ...
           mysql_select_db('siceiplum',$conectar) or
			   ...
			   HORA_MOVI,COD_USUARIO as COD_USUARIO,COD_ALM as COD_ALM, NUM_DOC_REF as NUM_DOC_REF, RIF_PROV as RIF_PROV, 
			   Nota as Nota from T_MOVI_HEDER where TIPO_MOVI = 'S' and FEC_MOVI = '$wg_fecha' order by FEC_MOVI"; 		         
//            Numero de Movimiento		   
		   	  if($tipo_objeto=='01'){ 
               $sql01="select NUM_MOVI as NUM_MOVI, CLASE_MOVI as CLASE_MOVI,FEC_MOVI as FEC_MOVI,HORA_MOVI as 
			   HORA_MOVI,COD_USUARIO as COD_USUARIO,COD_ALM as COD_ALM, NUM_DOC_REF as NUM_DOC_REF, RIF_PROV as RIF_PROV, 
			   Nota as Nota from T_MOVI_HEDER where TIPO_MOVI = 'S' and NUM_MOVI like '%$nombre_busqueda%' order by FEC_MOVI"; 		              }
//            Por Fecha			  
		   	  if($tipo_objeto=='02'){ 
               $sql01="select NUM_MOVI as NUM_MOVI, CLASE_MOVI as CLASE_MOVI,FEC_MOVI as FEC_MOVI,HORA_MOVI as 
			   HORA_MOVI,COD_USUARIO as COD_USUARIO,COD_ALM as COD_ALM, NUM_DOC_REF as NUM_DOC_REF, RIF_PROV as RIF_PROV, 
			   Nota as Nota from T_MOVI_HEDER where TIPO_MOVI = 'S' and FEC_MOVI like '%$nombre_busqueda%' order by FEC_MOVI"; 		              }
//            Codigo almacen			  
		   	  if($tipo_objeto=='03'){ 
               $sql01="select NUM_MOVI as NUM_MOVI, CLASE_MOVI as CLASE_MOVI,FEC_MOVI as FEC_MOVI,HORA_MOVI as 
			   HORA_MOVI,COD_USUARIO as COD_USUARIO,COD_ALM as COD_ALM, NUM_DOC_REF as NUM_DOC_REF, RIF_PROV as RIF_PROV, 
			   Nota as Nota from T_MOVI_HEDER where  TIPO_MOVI = 'S' and cod_alm like '%$nombre_busqueda%' order by FEC_MOVI"; 		              }
//           Documento Referencia			  
		   	  if($tipo_objeto=='04'){ 
               $sql01="select NUM_MOVI as NUM_MOVI, CLASE_MOVI as CLASE_MOVI,FEC_MOVI as FEC_MOVI,HORA_MOVI as 
			   HORA_MOVI,COD_USUARIO as COD_USUARIO,COD_ALM as COD_ALM, NUM_DOC_REF as NUM_DOC_REF, RIF_PROV as RIF_PROV, 
			   Nota as	Nota from T_MOVI_HEDER where TIPO_MOVI = 'S' and num_doc_ref like '%$nombre_busqueda%' order by FEC_MOVI"; 		              }
//            Clase de Movimiento			  
		   	  if($tipo_objeto=='05'){ 
               $sql01="select NUM_MOVI as NUM_MOVI, CLASE_MOVI as CLASE_MOVI,FEC_MOVI as FEC_MOVI,HORA_MOVI as 
			   HORA_MOVI,COD_USUARIO as COD_USUARIO,COD_ALM as COD_ALM, NUM_DOC_REF as NUM_DOC_REF, RIF_PROV as RIF_PROV , 
			   Nota as Nota from T_MOVI_HEDER where TIPO_MOVI = 'S' and clase_movi like '%$nombre_busqueda%' order by FEC_MOVI"; 		              }
...
OnClick=\"datos('$NUM_MOVI','$CLASE_MOVI','$FEC_MOVI','$COD_ALM','$NUM_DOC_REF','$solicit_sol','$nota','$desc_almacen','$desc_solicit');\"><td>$NUM_MOVI</td><td>  $CLASE_MOVI</td><td>$FEC_MOVI</td><td>$HORA_MOVI</td><td>$COD_USUARIO</td><td>$COD_ALM</td><td>$NUM_DOC_REF
				  </td></tr>";		
		    }
		   }
...
function datos(NUM_MOVI,CLASE_MOVI,FEC_MOVI,COD_ALM,NUM_DOC_REF,solicit_sol,nota,desc_almacen,desc_solicit){
