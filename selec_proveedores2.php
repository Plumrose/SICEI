...
function datos(rif_prov,Nomb_prov,cod_sap,tipo,nomb_tipo,observ){
  opener.document.form1.rif_prov.value=rif_prov;
  opener.document.form1.Nomb_prov.value=Nomb_prov;
  opener.document.form1.cod_sap_prov.value=cod_sap;
  opener.document.form1.tipo_prov.value=tipo;
  opener.document.form1.Desc_tipo_prov.value=nomb_tipo;
  opener.document.form1.observacion.value=observ;
  window.close();
}
</script>
...
<form id="form1" name="form1" method="post" action="selec_proveedores2.php">
  <p>&nbsp;</p>
  <p>&nbsp;</p>
<div id="busqueda">
  <table width="662">
...
//            Por Código SAP			  
           ...
           if(mysql_num_rows($resultado)>0) 
	       {
              ...
              { 
				  ...
				  $tipo=$registro['cod_tipo_prov'];
				  $tipo=str_replace(" ","&nbsp;",$tipo);
				  $observ=$registro['obser_prov'];
				  $observ=str_replace(" ","&nbsp;",$observ);
                  $sql02="select DESC_TIPO_PROV as DESC_TIPO_PROV from M_TIPOS_PROV where COD_TIPO_PROV = '$rif_prov'";	
                  $resultado2=mysql_query($sql02,$conectar) or die("Problemas en la Sentencia SQL".mysql_error());
                  $registro2=mysql_fetch_assoc($resultado2);
				  $nomb_tipo=$registro2['DESC_TIPO_PROV'];
				  $nomb_tipo=str_replace(" ","&nbsp;",$nomb_tipo);
                  echo "<tr OnMouseOver=Resaltar_On(this);' OnMouseOut='Resaltar_Off(this);' OnClick=\"datos('$rif_prov','$Nomb_prov','$cod_sap','$tipo','$nomb_tipo','$observ');\"><td>$rif_prov</td><td>$Nomb_prov</td><td>$cod_sap</td><td>$estatus</td></tr>";		
		    }
		   }
