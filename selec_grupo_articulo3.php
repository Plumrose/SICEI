...
function datos(cod,nombre,nombre_corto,ind_flag){
  opener.document.form1.cod_atributo.value=cod;
  opener.document.form1.desc_atributo2.value=nombre;
  if(ind_flag == 1)
  	opener.document.form1.otros_activos.checked=true;
  else
  	opener.document.form1.otros_activos.checked=false;
  opener.document.form1.nombre_corto.value=nombre_corto;
...
<form id="form1" name="form1" method="post" action="selec_grupo_articulo3.php">
  <p>&nbsp;</p>
  <p>&nbsp;</p>
<div id="busqueda">
  <table width="466">
    ...
    <table width="493" border="1" bordercolor="#CCCCCC" bgcolor="#FFFFFF" class="Estilo28">
      <tr>
        ...
        <?php
		   ...
		   if($nombre_busqueda==''){		
              $sql01="select cod_grupo_art as id, desc_grupo_art as des, ind_flag as ind_flag, nombre_corto as nombre_corto
			  from m_grupo_articulos order by cod_grupo_art";
		   }else{
              $sql01="select cod_grupo_art as id, desc_grupo_art as des, ind_flag as ind_flag, nombre_corto as nombre_corto 
			  from m_grupo_articulos where desc_grupo_art like '%$nombre_busqueda%' order by cod_grupo_art";
		   }	  
           ...
           if(mysql_num_rows($resultado)>0) 
	       {
              ...
              { 
                  ...
                  $ind_flag=$registro['ind_flag'];
				  $ind_flag=str_replace(" ","&nbsp;",$ind_flag);
                  $nombre_corto=$registro['nombre_corto'];
				  $nombre_corto=str_replace(" ","&nbsp;",$nombre_corto);
				  
				  echo "<tr OnMouseOver=Resaltar_On(this);' OnMouseOut='Resaltar_Off(this);' OnClick=\"datos('$cod','$nombre','$nombre_corto','$ind_flag');\"><td>$cod</td><td>$nombre</td></tr>";
   		      }
		   } 
