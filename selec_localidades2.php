...
function datos(dunidad,cunidad,grupo,nombre){
  opener.document.form1.des_localidad.value=dunidad;
  opener.document.form1.cod_localidad.value=cunidad;
  opener.document.form1.cod_grupo.value=grupo;
  opener.document.form1.des_grupo.value=nombre;
  window.close();
}
</script>
...
<div id="Layer1">
  <div align="center" class="Estilo28">
    <p class="Estilo41">Listado de Localidades   </p>
  </div>
</div>
<form id="form1" name="form1" method="post" action="selec_localidades2.php">
  <p>&nbsp;</p>
  <p>&nbsp;</p>
<div id="busqueda">
  <table width="519">
      ...
      <tr>
        ...
        <?php
           ...
           mysql_select_db('siceiplum',$conectar) or
              ...
              $sql01="select cod_localidad as cod_localidad, desc_localidad as desc_localidad, cod_agrupacion as grupo from m_localidades";
		   	  if($tipo_objeto=='01'){ 
                    $sql01="select cod_localidad as cod_localidad, desc_localidad as desc_localidad, cod_agrupacion as grupo from m_localidades where 
					cod_localidad like '%$nombre_busqueda%'";
		      }
		   	  if($tipo_objeto=='02'){ 	
                    $sql01="select cod_localidad as cod_localidad, desc_localidad as desc_localidad, cod_agrupacion as grupo from m_localidades where 
					desc_localidad like '%$nombre_busqueda%'";
		      }
              ...
              { 
				  $dunidad=$registro['desc_localidad'];
				  $dunidad=str_replace(" ","&nbsp;",$dunidad);
				  $cunidad=$registro['cod_localidad'];
				  $cunidad=str_replace(" ","&nbsp;",$cunidad);
				  $grupo=$registro['grupo'];
				  $grupo=str_replace(" ","&nbsp;",$grupo);
				  $sql09="select des_agrupacion as des_agrupacion from m_grupo_localidad where cod_agrupacion='$grupo'";
				  $resul9=mysql_query($sql09,$conectar) or die("Problemas en la Sentencia SQL nro9".mysql_error());
                  $reg9=mysql_fetch_assoc($resul9);
                  $nombre= $reg9["des_agrupacion"];

				  echo "<tr OnMouseOver=Resaltar_On(this);' OnMouseOut='Resaltar_Off(this);' 
				  OnClick=\"datos('$dunidad','$cunidad','$grupo','$nombre');\"><td>$cunidad</td><td>$dunidad</td></tr>";
   		      }
		   } 
